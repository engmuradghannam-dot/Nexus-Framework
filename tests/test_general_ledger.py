...
class TestGeneralLedger:
    ...
    def test_posting_journal_entry_updates_account_balances(self, authenticated_client, company):
        ...
        je = authenticated_client.post('/api/v1/journal-entries/', {
            'company': company.id, 'entry_number': 'JE-1', 'posting_date': '2026-07-06',
            'debit_account': cash['id'], 'credit_account': sales['id'], 'amount': 5000,
        }).data
        resp = authenticated_client.patch(f'/api/v1/journal-entries/{je["id"]}/', {'status': 'Submitted'})
        assert resp.status_code == 200

        cash_after = authenticated_client.get(f'/api/v1/accounts/{cash["id"]}/').data
        sales_after = authenticated_client.get(f'/api/v1/accounts/{sales["id"]}/').data
    ...
    def test_trial_balance_is_balanced(self, authenticated_client, company):
        ...
        authenticated_client.patch(f'/api/v1/journal-entries/{je["id"]}/', {'status': 'Submitted'})

        resp = authenticated_client.get(f'/api/v1/accounts/trial_balance/?company={company.id}')
        assert resp.status_code == 200
    ...
    def test_financial_statements_balance_sheet_balances(self, authenticated_client, company):
        ...
        resp = authenticated_client.get(f'/api/v1/accounts/financial_statements/?company={company.id}')
        assert resp.status_code == 200
        assert float(resp.data['income_statement']['net_income']) == 2000.0
        assert resp.data['balance_sheet']['balanced'] is True

    def test_cannot_post_to_group_account(self, authenticated_client, company):
        group_acc = authenticated_client.post('/api/v1/accounts/', {
            'company': company.id, 'account_name': 'Assets (Header)', 'account_number': '1000',
            'account_type': 'Asset', 'root_type': 'Asset', 'is_group': True,
        }).data
        sales = authenticated_client.post('/api/v1/accounts/', {
