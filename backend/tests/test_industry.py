...
from apps.industry.models import IndustryVertical, VerticalTemplate, Company, Sector, Metric
...
class TestIndustryVerticalAPI:
    ...
    def test_vertical_companies_action(self, auth_client, sample_vertical, sample_company):
    ...
    def test_clone_from_template(self, auth_client, sample_vertical, sample_template):
        url = reverse('vertical-clone-from-template', kwargs={'pk': str(sample_vertical.id)})
        response = auth_client.post(url, {'template_id': str(sample_template.id)}, format='json')
        assert response.status_code == 200
