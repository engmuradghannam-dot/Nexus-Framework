...
class NexusAdminSite(AdminSite):
    ...
    def get_app_list(self, request, app_label=None):
        ...
        model_icons = {row[0]: row[1] for row in self.MODEL_ICONS}
        ...
        app_list.sort(
            key=lambda x: (
                self.MODULE_ORDER.index(x["app_label"])
                if x["app_label"] in self.MODULE_ORDER
                else 999
            )
        )
        return app_list
    ...
    def index(self, request, extra_context=None):
        ...
        module_icons_array = [
            {"label": row[0], "icon": row[1], "color": row[2], "description": row[3]}
            for row in self.MODULE_ICONS
        ]

        model_icons_array = [
            {"name": row[0], "icon": row[1]} for row in self.MODEL_ICONS
        ]
