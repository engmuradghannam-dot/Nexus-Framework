...
export default function PurchaseOrders() {
  const { data, isLoading } = useQuery({ queryKey: ['purchaseOrders'], queryFn: fetchPurchaseOrders });
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manage supplier orders and track deliveries</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 border-b">
          <div className="relative max-w-md">
            ...
            <input type="text" placeholder="Search POs..." className="w-full pl-10 pr-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>
        </div>
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">PO Number</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {data?.results?.map((po) => (
              <tr key={po.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-gray-700">{po.supplier_name || po.supplier}</td>
                <td className="px-6 py-4 text-gray-600 text-sm">{po.transaction_date}</td>
                <td className="px-6 py-4 font-medium">${po.grand_total}</td>
