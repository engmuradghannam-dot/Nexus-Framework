...
export default function SalesOrders() {
  const { data, isLoading } = useQuery({ queryKey: ['salesOrders'], queryFn: fetchSalesOrders });
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 border-b">
          <div className="relative max-w-md">
            ...
            <input type="text" placeholder="Search SOs..." className="w-full pl-10 pr-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>
        </div>
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">SO Number</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {data?.results?.map((so) => (
              <tr key={so.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-gray-700">{so.customer_name || so.customer}</td>
                <td className="px-6 py-4 text-gray-600 text-sm">{so.transaction_date}</td>
                <td className="px-6 py-4 font-medium">${so.grand_total}</td>
                <td className="px-6 py-4">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                    so.status === 'Submitted' || so.status === 'Delivered' ? 'bg-green-100 text-green-700' :
