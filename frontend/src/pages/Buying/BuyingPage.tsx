...
import { Store, Plus, RefreshCw, Send, PackageCheck, XCircle, Trash2, AlertTriangle, CheckCircle } from 'lucide-react';
...
import { buyingApi, suppliersApi, itemsApi, getWarehouses } from '../../services/api';

const fmt = (n: any) => Number(n || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });

const STATUS_LABEL: Record<string, string> = { Draft: 'مسودة', Submitted: 'مُعتمد', Received: 'تم الاستلام', Cancelled: 'ملغى' };
const STATUS_VARIANT: Record<string, any> = { Draft: 'warning', Submitted: 'info', Received: 'success', Cancelled: 'error' };
...
export default function BuyingPage() {
  ...
  const reload = () => {
    setLoading(true);
    buyingApi.orders().then(setOrders).catch(() => {}).finally(() => setLoading(false));
  };
  ...
  const loadLines = (poId: number) => buyingApi.items(poId).then(setLines).catch(() => setLines([]));

  const openNew = () => {
    setOrder({ supplier: '', po_number: '', transaction_date: new Date().toISOString().slice(0, 10), warehouse: '' });
    setLines([]);
    setError(''); setNotice('');
    setPanelOpen(true);
  };
  ...
  const openOrder = async (row: any) => {
    const full = await buyingApi.getOrder(row.id);
    setOrder(full);
    setError(''); setNotice('');
    setPanelOpen(true);
    loadLines(row.id);
  ...
  const createDraft = async () => {
    try {
      const created = await buyingApi.createOrder(order);
      setOrder(created);
  ...
  const addLine = async () => {
    ...
    try {
      await buyingApi.addItem({ purchase_order: order.id, item: newLine.item, qty: newLine.qty, rate: newLine.rate });
      setNewLine({ item: '', qty: 1, rate: 0 });
      await loadLines(order.id);
      const refreshed = await buyingApi.getOrder(order.id);
      setOrder(refreshed);
      reload();
    } catch (e: any) {
      setError(formatApiError(e));
  ...
  const transition = async (status: string) => {
    setError(''); setNotice('');
    try {
      const updated = await buyingApi.updateOrder(order.id, { status });
      setOrder(updated);
      setNotice(status === 'Received' ? 'تم استلام المخزون وتحديث الأمر.' : `تم تحويل الحالة إلى ${STATUS_LABEL[status]}.`);
      reload();
    } catch (e: any) {
      setError(formatApiError(e));
    }
  ...
  return (
    <div className="min-h-full" dir="rtl">
      <FluentCommandBar
        title="أوامر الشراء" subtitle="Purchase Orders — دورة كاملة من المسودة حتى الاستلام"
        commands={[
          { id: 'refresh', label: 'تحديث', icon: <RefreshCw size={16} />, variant: 'secondary', onClick: reload },
          { id: 'new', label: 'أمر شراء جديد', icon: <Plus size={16} />, variant: 'primary', onClick: openNew },
        ]}
      />
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-3 gap-4">
          <FluentStatsCard title="عدد الأوامر" value={stats.total} icon={<Store size={20} />} color="blue" />
          <FluentStatsCard title="مفتوحة" value={stats.open} icon={<Store size={20} />} color="orange" />
          <FluentStatsCard title="إجمالي القيمة" value={fmt(stats.value)} icon={<Store size={20} />} color="purple" />
        </div>
        ...
        <FluentTable
          ...
          columns={[
            { key: 'po_number', label: 'رقم الأمر', sortable: true },
      ...
      <FluentPanel
        ...
        title={order?.id ? `أمر شراء ${order.po_number}` : 'أمر شراء جديد'}
          ...
          <div className="flex items-center justify-between">
            ...
            {order?.id && order.status === 'Submitted' && (
              <div className="flex gap-2">
                ...
                <button onClick={() => transition('Received')} className="flex items-center gap-2 px-4 py-2 text-sm text-white bg-[#107c10] rounded-sm hover:bg-[#0e6e0e]"><PackageCheck size={14} /> استلام المخزون</button>
              </div>
            )}
          </div>
        }
      >
        ...
        {order && !order.id && (
          <div className="space-y-3">
            <FluentFormField label="المورّد" required>
              <FluentSelect value={order.supplier || ''} onChange={(e) => setOrder({ ...order, supplier: e.target.value })}>
                <option value="">— اختر مورّداً —</option>
                {suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
              </FluentSelect>
            </FluentFormField>
            <FluentFormField label="رقم أمر الشراء" required>
              <FluentInput value={order.po_number || ''} onChange={(e) => setOrder({ ...order, po_number: e.target.value })} placeholder="PO-2026-001" />
            </FluentFormField>
            ...
            <FluentFormField label="المستودع" hint="مطلوب قبل الاستلام">
        ...
        {order?.id && (
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-[#605e5c]">المورّد: </span><span className="font-medium">{order.supplier_name}</span></div>
